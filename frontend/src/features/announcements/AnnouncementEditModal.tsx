...
const AnnouncementEditModal: React.FC<AnnouncementEditModalProps> = ({ announcement, isOpen, onClose, onAnnouncementUpdated, isSchoolScope = false }) => {
  const { courseId, announcementId } = useParams<{ courseId: string; announcementId: string }>();
  const queryClient = useQueryClient();
  const [title, setTitle] = useState(announcement.title);
  const [description, setDescription] = useState(announcement.description);
  const [newAttachments, setNewAttachments] = useState<File[]>([]);
  const [attachmentsToKeep, setAttachmentsToKeep] = useState<string[]>(announcement.attachements || []);
  ...
  useEffect(() => {
    ...
    setAttachmentsToKeep(announcement.attachements || []);
  }, [announcement]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!announcementId || (isSchoolScope && !announcementId) || (!isSchoolScope && !courseId)) return;
    ...
    try {
      let updatedAnnouncementData;
      if (isSchoolScope) {
        await apiService.updateSchoolAnnouncement(announcementId, formData);
        updatedAnnouncementData = await apiService.getSchoolAnnouncement(announcementId);
        await queryClient.invalidateQueries({ queryKey: ['dashboard-announcements'] });
        await queryClient.invalidateQueries({ queryKey: ['school-announcement', announcementId] });
      } else {
        await apiService.updateCourseAnnouncement(courseId!, announcementId, formData);
        updatedAnnouncementData = await apiService.getCourseAnnouncement(courseId!, announcementId);
        await queryClient.invalidateQueries({ queryKey: ['course-feed', courseId] });
        await queryClient.invalidateQueries({ queryKey: ['announcement', courseId, announcementId] });
      }
      onAnnouncementUpdated(updatedAnnouncementData);
